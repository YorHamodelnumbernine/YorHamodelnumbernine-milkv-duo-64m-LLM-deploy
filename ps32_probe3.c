/* ps32_probe3.c — decisive: zero entire neuron mem, run ps32 matmul,
   scan full 32KB lmem for any nonzero bytes. If nothing changes, TIU
   writes no ps32 output anywhere in lmem.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define LMEM_SZ 32768
#define OUT_OFF 8192

static void run_scan(const char *name, int ps32_mode, int res_is_int8,
  ...
  memset(va, 0, NEURON_SZ);          /* zero ALL */
  ...
  printf("== %s == ps32_mode=%d res_is_int8=%d alloc_fmt=%s\n", name, ps32_mode,
         res_is_int8, alloc_fmt==FMT_I8?"I8":alloc_fmt==FMT_BF16?"BF16":"other");
  printf("  left lmem addr=%u right=%u res=%u (ps32)\n",
         ml_l->start_address, ml_r->start_address, ml_res->start_address);
  ...
  /* copy back full 32KB lmem region starting at offset 0 in lmem.
     lmem 0..32768 maps to neuron va+0 .. va+32768 (base reg 0). */
  bmk1822_tdma_l2g_general_copy(bmk, &(bmk1822_tdma_l2tg_general_copy_param_t){
    .src_address = 0, .dst_base_reg_index = 0,
    .dst_address = OUT_OFF, .bytes = LMEM_SZ });
  ...
  uint8_t *lm = (uint8_t*)(va + OUT_OFF);
  /* scan nonzero */
  int nz = 0; int first_nz[8] = {0}; int first_idx = 0;
  for (int i = 0; i < LMEM_SZ; i++) {
    if (lm[i]) {
      if (nz < 8) first_nz[nz] = i;
      nz++;
    }
  }
  printf("  nonzero bytes in 32KB lmem: %d", nz);
  if (nz > 0) {
    printf("  first nonzero offsets:");
    for (int j = 0; j < nz && j < 8; j++) printf(" %d", first_nz[j]);
  }
  printf("\n");
  if (nz > 0) {
    printf("  around res addr %u: ", ml_res->start_address);
    for (int i = 0; i < 16; i++) printf("%02x ", lm[ml_res->start_address + i]);
    printf("\n");
  }
  /* expect */
...
int main(void) {
  printf("== ps32 lmem scan probe ==\n");
  run_scan("scan_ps32m1_i8", 1, 1, FMT_I8, 32, 100, 100);
  run_scan("scan_ps32m2_bf16", 2, 0, FMT_BF16, 32, 100, 100);
  run_scan("scan_nrm_i8", 0, 1, FMT_I8, 32, 100, 100);
  return 0;
}
