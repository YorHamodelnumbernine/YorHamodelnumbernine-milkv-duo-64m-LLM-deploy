/* ps32_probe9.c — full byte-plane layout map for [M=1,N=896] ps32 buffer.
   Run 1: all columns value +1 -> byte0 planes show as 0x01.
   Run 2: all columns value -1 (0xFFFFFFFF) -> all 4 byte-planes show as 0xFF.
   Prints all offsets of 0x01 / 0xFF across the full readback.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void run_all(const char *tag, int val) {
  ...
  uint32_t rd = 4096;
  memset(va + 4096, 0, rd); CVI_RT_MemFlush(rt, mem);
  bmk1822_tdma_g2l_general_copy(bmk, &(bmk1822_tdma_tg2l_general_copy_param_t){
    .src_base_reg_index = 0, .src_address = 4096, .dst_address = ml_res->start_address, .bytes = rd });
  ...
  printf("[%s] offsets of 0x%02x in %u-byte readback (count): ", tag, pat, rd);
  int cnt = 0; int prev = -2; int run_start = -1;
  for (uint32_t i = 0; i < rd; i++) {
    if (r[i] == pat) {
      cnt++;
      if (i != prev+1) { if (run_start>=0 && prev>=run_start) printf("[%u..%u] ", run_start, prev); run_start = i; }
      prev = i;
    }
  }
  if (run_start>=0) printf("[%u..%u] ", run_start, prev);
  printf(" total=%d\n", cnt);
out:
...
int main(void) {
  printf("== ps32 byte-plane layout map ==\n");
  run_all("val+1", 1);
  run_all("val-1", -1);
  return 0;
}
