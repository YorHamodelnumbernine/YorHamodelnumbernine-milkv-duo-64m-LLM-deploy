...
推荐：**主路径采用 Path A 两遍法（KG=32=G ps32-free；2026-08-13 CEO 确认，引擎 spec 已锁 KG=32）**。
> **KG 口径修正（2026-08-13 CEO 定稿）**：本句此前"KG=128 ps32-free"为顺延假设、**不成立**——
> Path A 组结构（K-aligned INT4 G32）锁死 KG=32（group_int4 基线 gap 1.923/1.105/0.485，
> REPORT_WS_DQ_VETO §2）。KG=128 降提交方案仅作候选，待 Path A host 补扫确认后评估。
