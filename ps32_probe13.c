/* ps32_probe13.c — column-width limit + submit latency for ps32 K=32 slices.
   Test N=112, 224, 896. Report how many columns' byte0 are present in the
   ps32 buffer (identity trick: all cols = 1). Also time the submit.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void run_N(const char *tag, int N, int K) {
  ...
  printf("[%s] N=%d res addr=%u psz=%u sh{n=%u,c=%u,w=%u,col=%u} str{n=%u,c=%u,h=%u}\n",
         tag, N, ml_res->start_address, psz, ml_res->shape.n, ml_res->shape.c,
  ...
  for (uint32_t off = ml_res->start_address; off < ml_res->start_address + psz; off++)
    if (r[off] == 0x01) present++;
  printf("[%s] submit time=%.3f ms | 0x01 bytes in res region=%d (of expected %d cols)\n",
         tag, ms, present, N);
out:
...
int main(void) {
  printf("== ps32 column-width limit probe ==\n");
  run_N("N112", 112, 32);
  run_N("N224", 224, 32);
  run_N("N448", 448, 32);
  run_N("N896", 896, 32);
