/* TPU depthwise point-wise (1x1) conv — bmk1822 API.
   IC=OC=2, 1x1 kernel, identity test. */
...
int main() {
  ...
  /* Depthwise 1x1 weight: per-channel identity, oc0->ic0=1, oc1->ic1=1 */
  static const int8_t weight[2] = {1, 1};  /* {oc0, oc1} each maps to its channel */
  static const int8_t expected[8] = {1,2, 3,4,  5,6, 7,8};

  memcpy(neuron_va, ifmap, 8);
  memcpy(neuron_va+32, weight, 2);
  ...
  /* Depthwise weight: {1, OC, KH, KW} with stride_type_2 */
  bmk1822_tensor_lmem_t *tl_w  = bmk1822_lmem_alloc_tensor(bmk, (bmk1822_tensor_lmem_shape_t){1,2,1,1}, FMT_I8, 1);
  ...
  bmk1822_tensor_tgmem_t tg_w = {
    .base_reg_index = 0, .start_address = 32, .fmt = FMT_I8, .shape = {1,2,1,1},
  ...
  printf("  [dw_pt_conv_bmk] %s  time: %.2f us\n", errs?"FAIL":"OK", t_ns/1000.0);
