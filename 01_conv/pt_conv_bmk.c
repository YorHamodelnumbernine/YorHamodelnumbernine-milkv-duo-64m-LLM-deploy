/* TPU point-wise (1x1) regular conv — bmk1822 API.
   IC=OC=2, 1x1 kernel, identity test: ifmap comes through unchanged.
   Weight: {2,2,1,1} = {IC,OC,KH,KW}, stride_type_2. */
...
int main() {
  ...
  /* IC=2, IH=IW=2: ch0=[1,2;3,4], ch1=[5,6;7,8] */
  static const int8_t ifmap[8] = {1,2, 3,4,  5,6, 7,8};
  /* Weight identity: oc0→ich0 (1), oc1→ich1 (1), cross-terms=0 */
  static const int8_t weight[4] = {1,0, 0,1};  /* {ic=0,oc=0}=1, {0,1}=0, {1,0}=0, {1,1}=1 */
  static const int8_t expected[8] = {1,2, 3,4,  5,6, 7,8};

  memcpy(neuron_va, ifmap, 8);
  memcpy(neuron_va+32, weight, 4);
  ...
  bmk1822_tensor_lmem_t *tl_if = bmk1822_lmem_alloc_tensor(bmk, (bmk1822_tensor_lmem_shape_t){1,2,2,2}, FMT_I8, 1);
  bmk1822_tensor_lmem_t *tl_of = bmk1822_lmem_alloc_tensor(bmk, (bmk1822_tensor_lmem_shape_t){1,2,2,2}, FMT_I8, 1);
  bmk1822_tensor_lmem_t *tl_w  = bmk1822_lmem_alloc_tensor(bmk, (bmk1822_tensor_lmem_shape_t){2,2,1,1}, FMT_I8, 1);
  if (!tl_if||!tl_of||!tl_w) { fprintf(stderr,"alloc fail\n"); goto out_bmk; }
  tl_w->stride.n = 1;
  tl_w->cmprs_fmt = FMT_I8;

  bmk1822_tensor_tgmem_t tg_if = {
    .base_reg_index = 0, .start_address = 0, .fmt = FMT_I8, .shape = {1,2,2,2},
    .stride = bmk1822_tensor_tgmem_default_stride((bmk1822_tensor_tgmem_shape_t){1,2,2,2}, FMT_I8),
  };
  bmk1822_tensor_tgmem_t tg_w = {
    .base_reg_index = 0, .start_address = 32, .fmt = FMT_I8, .shape = {2,2,1,1},
    .stride = bmk1822_tensor_tgmem_default_stride((bmk1822_tensor_tgmem_shape_t){2,2,1,1}, FMT_I8),
  };
  bmk1822_tensor_tgmem_t tg_of = {
    .base_reg_index = 0, .start_address = 64, .fmt = FMT_I8, .shape = {1,2,2,2},
    .stride = bmk1822_tensor_tgmem_default_stride((bmk1822_tensor_tgmem_shape_t){1,2,2,2}, FMT_I8),
  };
  ...
  if (!bmk1822_tiu_convolution(bmk, &p)) { fprintf(stderr,"conv rejected\n"); goto out_bmk; }
  ...
  errs = 0;
  for (int i = 0; i < 8; i++) { if (v[64+i] != expected[i]) errs++; }
  printf("  [pt_conv_bmk] %s  time: %.2f us\n", errs?"FAIL":"OK", t_ns/1000.0);
