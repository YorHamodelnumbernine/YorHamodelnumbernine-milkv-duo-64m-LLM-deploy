/* ps32_probe20.c — width-limit map for forced c=1 ps32.  For each N:
   (A) g2l right [K=32,N] forced shape, readback, FIXED signed compare -> g2l complete?
   (B) single ps32 matmul, readback res with byte-plane stride=N -> exact match?
   Separates TDMA (g2l) vs TIU (matmul) width limits.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void run_forced(int K, int N) {
  ...
  if (!ml_res) { printf("[N%3d] alloc fail\n", N); return; }
  ...
  if (!bmk1822_tiu_matrix_multiplication(bmk, &p)) { printf("[N%3d] REJECTED\n", N); return; }
  ...
  /* (A) right g2l completeness — fixed signed compare */
  int8_t *rb = (int8_t*)(va + OUT_OFF);
  int bad = 0;
  for (int k = 0; k < K; k++)
    for (int n = 0; n < N; n++)
      if (rb[k*N + n] != right[k*N + n]) bad++;
  printf("[N%3d] psz=%u rsz=%u | g2l right: %d/%d bad (%s) | ", N, psz, rsz, bad, K*N,
         bad == 0 ? "COMPLETE" : "TRUNCATED");

  /* (B) res stride=N readback */
  uint8_t *r = (uint8_t*)(va + OUT_OFF + 65536);
  int ok = 0, nbad = 0;
  for (int n = 0; n < N; n++) {
    int32_t v = 0; int valid = 1;
    for (int b = 0; b < 4; b++) {
      uint64_t off = (uint64_t)b*N + n;
      if (off >= 32768) { valid = 0; break; }
      v |= ((int32_t)r[off]) << (8*b);
    }
    if (!valid) nbad++;
    else if (v == host[n]) ok++; else nbad++;
  }
  printf("ps32 stride=N: ok=%d/%d bad=%d\n", ok, N, nbad);
  if (ok == 0 && N > 16)
    printf("  col0 bytes @0,%u,%u,%u: %02x %02x %02x %02x (host=%d)\n",
           (uint32_t)N, 2*(uint32_t)N, 3*(uint32_t)N, r[0], r[N], r[2*N], r[3*N], host[0]);
  free(host);
  bmk1822_cleanup(bmk); CVI_RT_MemFree(rt, mem); CVI_RT_DeInit(rt);
}

int main(void) {
  int ns[] = {16, 32, 64, 112, 128, 144, 160, 192, 224, 256};
  printf("== probe20: forced c=1 ps32 width-limit map (K=32) ==\n");
