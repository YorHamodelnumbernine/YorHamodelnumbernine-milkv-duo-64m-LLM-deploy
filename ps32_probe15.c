/* ps32_probe15.c — decisive chunk-width sweep for ps32 int32 output.
   Engine-style dense right load: for chunk width cn, build dense right[K,cn]
   in neuron, g2l with stride cn, run ps32 matmul [1,K]x[K,cn], read back the
   FULL ps32 buffer, reconstruct int32 per column, count exact matches vs host.

   This determines the max usable output width per ps32 matmul call.
   Chunk widths tested: 8, 16, 32, 64, 112, 128.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void run_cn(int K, int N) {
  ...
  if (!bmk1822_tiu_matrix_multiplication(bmk, &p)) { printf("[cn=%d] REJECTED\n", N); goto out; }
  ...
  bmk1822_tdma_l2g_general_copy(bmk, &(bmk1822_tdma_l2tg_general_copy_param_t){
    ...
    .dst_address = OUT_OFF, .bytes = 16384 });
  ...
  /* Reconstruct: try byte-plane stride = 16 (probe6 layout): byte b of col n at
     b*16 + n.  For w>16, try plane-groups of 16: col n -> byte b at b*plane_stride
     + n, where plane_stride = psz/4? or N?  Try multiple hypotheses. */
  const uint32_t strides[] = {16, (uint32_t)N, (uint32_t)(psz/4), 1, 8, 32, 64, 112};
  int best = -1; uint32_t best_stride = 0;
  for (unsigned si = 0; si < sizeof(strides)/sizeof(uint32_t); si++) {
    uint32_t st = strides[si];
    if (st == 0) continue;
    int ok = 0, bad = 0;
    for (int n = 0; n < N; n++) {
    ...
    if (ok > best) { best = ok; best_stride = st; }
  }
  printf("  -> best stride=%u ok=%d/%d\n", best_stride, best, N);

  /* dump first 4 plane bytes for col 0 to help debug */
  printf("  col0 bytes at stride16: %02x %02x %02x %02x (host=%d)\n",
         r[0], r[16], r[32], r[48], host[0]);
  printf("  col0 bytes at strideN(%u): %02x %02x %02x %02x\n",
         (uint32_t)N, r[0], r[N], r[2*N], r[3*N]);
out:
  free(host);
...
int main(void) {
  printf("== probe15: ps32 chunk-width sweep (K=32, dense right) ==\n");
  run_cn(32, 8);
  run_cn(32, 16);
  run_cn(32, 32);
  run_cn(32, 64);
