#!/usr/bin/env python3
"""emu_group_int4.py — Path A INT4 G32 K-aligned + two-pass rshift.

The proposed hardware config: store INT4 G32 K-aligned weights (SD 8.39MB/layer),
CPU dequant to per-group INT8 (values -8..7, scale per (group,col)) before the
chunk matmul, two-pass rshift, fp32 accumulate with per-(group,col) scale.

Tests whether INT4 weight error + int8 output rounding (two-pass) together hold 3/3.
"""
import os, sys, json, math
import numpy as np

HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
import emu_chunk8 as C8
import emu_perrow as EP
from emu_perrow import (D, H, KVH, HD, L, F, V, DKV, GROUPS, PROMPTS, REF,
                        matmul_rshift, int8_round_div, precompute_rope,
                        rope, rms_norm, silu, LS, LS_PL)
from qwen_emu import ROPE_THETA, EPS

PRE = os.path.join(HERE, "..", "qwen_int4")
sys.path.insert(0, PRE)
from sf_io import SF
import qwen_emu as QE

G = 32


def quant_groups_i4(W_hf):
    """[N,K] -> (q4 int8 stored -8..7, gscale [K/G,N]) K-aligned INT4."""
    Wt = np.ascontiguousarray(W_hf.T.astype(np.float32))   # [K,N]
    K, N = Wt.shape
    q = np.zeros((K, N), dtype=np.int8)
    gs = np.zeros((K // G, N), dtype=np.float32)
    for g in range(0, K, G):
        seg = Wt[g:g + G, :]
        mx = np.max(np.abs(seg), axis=0)
        sc = np.maximum(mx / 7.0, 1e-12)                   # INT4 symmetric scale
        q[g:g + G, :] = np.clip(np.round(seg / sc), -8, 7).astype(np.int8)
        gs[g // G, :] = sc
    return q, gs


def matmul_rshift_w(K, wmax):
    r = 0
    md = K * 127 * wmax
    while (md >> r) > 127:
        r += 1
    return r


def matmul_group_i4(x_i8, w_i8, gscale, sc_row, KG=G):
    M, K = x_i8.shape
    N = w_i8.shape[1]
    wmax = int(np.max(np.abs(w_i8)))
    out = np.zeros((M, N), dtype=np.float64)
    x = x_i8.astype(np.int32)
    w = w_i8.astype(np.int32)
    for g in range(0, K, KG):
        kc = min(KG, K - g)
        acc = x[:, g:g + kc] @ w[g:g + kc, :]
        rsafe = max(matmul_rshift_w(kc, wmax) - 3, 4)      # int4-aware safe rshift
        p1 = int8_round_div(acc, rsafe)
        est = int(np.max(np.abs(p1))) * (1 << rsafe)
        r = 0
        if est > 1e-6:
            r = int(math.ceil(math.log2(est / 127.0)))
            r = max(r, 0)
        p2 = int8_round_div(acc, r).astype(np.float64) * (1 << r)
        out += p2 * gscale[g // KG, :].reshape(1, -1)
    return out.astype(np.float32) * sc_row[:, None]


def load_i4():
    sf = SF(os.path.join(PRE, "model", "model.safetensors"))
    embed, esc, frms, _ = QE.load_weights()
    MATS = [("q_proj", "self_attn", D, D), ("k_proj", "self_attn", D, DKV),
            ("v_proj", "self_attn", D, DKV), ("o_proj", "self_attn", D, D),
            ("up_proj", "mlp", D, F), ("gate_proj", "mlp", D, F),
            ("down_proj", "mlp", F, D)]
    layers = []
    for l in range(L):
        p = f"model.layers.{l}"
        mats, gsc = {}, {}
        for name, grp, K, N in MATS:
            w = sf.get(f"{p}.{grp}.{name}.weight")
            q, gs = quant_groups_i4(w)
            key = {"q_proj": "Wq", "k_proj": "Wk", "v_proj": "Wv", "o_proj": "Wo",
                   "up_proj": "up", "gate_proj": "gate", "down_proj": "down"}[name]
            mats[key] = q
            gsc[key] = gs
        bias = np.concatenate([sf.get(f"{p}.self_attn.{n}.bias").astype(np.float32)
                               for n in ("q_proj", "k_proj", "v_proj")])
        layers.append((sf.get(f"{p}.input_layernorm.weight").astype(np.float32), mats,
                       sf.get(f"{p}.post_attention_layernorm.weight").astype(np.float32), bias, gsc))
    return embed, esc, frms, layers


def forward(ids, embed, esc, frms, layers):
    seq = len(ids)
    cos, sin = precompute_rope(max(64, seq + 8), HD, ROPE_THETA)
    x = np.zeros((seq, D), np.float32)
    for i, t in enumerate(ids):
        t = 0 if (t < 0 or t >= V) else t
        x[i] = embed[t].astype(np.float32) * esc[t]
    for rms_attn, mats, rms_ffn, bias, gsc in layers:
        bq, bk, bv = bias[:D], bias[D:D + DKV], bias[D + DKV:]
        h = rms_norm(x, rms_attn)
        x_i8, sc_row = C8.quant_per_row(h)
        q = matmul_group_i4(x_i8, mats["Wq"], gsc["Wq"], sc_row) + bq
        k = matmul_group_i4(x_i8, mats["Wk"], gsc["Wk"], sc_row) + bk
        v = matmul_group_i4(x_i8, mats["Wv"], gsc["Wv"], sc_row) + bv
        q = q.reshape(seq, H, HD); k = k.reshape(seq, KVH, HD); v = v.reshape(seq, KVH, HD)
        for s in range(seq):
            for hh in range(H):
                q[s, hh] = rope(q[s, hh], s, cos, sin, HD)
            for hh in range(KVH):
                k[s, hh] = rope(k[s, hh], s, cos, sin, HD)
        attn = np.zeros((seq, H, HD), np.float32)
        for hh in range(H):
            kvh = hh // GROUPS
            qh = q[:, hh, :]; kh = k[:, kvh, :]; vh = v[:, kvh, :]
            lg = (qh @ kh.T) * (1.0 / math.sqrt(HD))
            mask = np.triu(np.ones((seq, seq)), 1).astype(bool)
            lg = np.where(mask, -1e30, lg)
            pr = np.exp(lg - lg.max(-1, keepdims=True))
            pr /= pr.sum(-1, keepdims=True)
            attn[:, hh, :] = pr @ vh
        attn = attn.reshape(seq, D)
        a_i8, sc_a = C8.quant_per_row(attn)
        x = x + matmul_group_i4(a_i8, mats["Wo"], gsc["Wo"], sc_a)
        h = rms_norm(x, rms_ffn)
        x_i8, sc_row = C8.quant_per_row(h)
        up = matmul_group_i4(x_i8, mats["up"], gsc["up"], sc_row)
        gate = matmul_group_i4(x_i8, mats["gate"], gsc["gate"], sc_row)
        mid = up * silu(gate)
        out = np.zeros((seq, D), np.float32)
        for kc in range(0, F, 1024):
            kcn = min(1024, F - kc)
            m_i8, sc_m = C8.quant_per_row(mid[:, kc:kc + kcn])
            wc = mats["down"][kc:kc + kcn, :]
            gsw = gsc["down"][kc // G:(kc + kcn) // G, :]
            out += matmul_group_i4(m_i8, wc, gsw, sc_m)
        x = x + out
    h = rms_norm(x, frms)
    return h[-1] @ (embed.astype(np.float32) * esc.reshape(-1, 1)).T


def main():
    print("[A-int4] loading INT4 G32 K-aligned...", flush=True)
    embed, esc, frms, layers = load_i4()
    print("[A-int4] loaded.", flush=True)
    tok = None
    try:
        from transformers import AutoTokenizer
        tok = AutoTokenizer.from_pretrained(os.path.join(PRE, "model"), trust_remote_code=True)
    except Exception as e:
        print("tok fail", e)
    passed = 0
    row = {}
    for p in PROMPTS:
        ids = tok.encode(p, add_special_tokens=False) if tok else [0]
        lg = forward(ids, embed, esc, frms, layers)
        t5 = np.argsort(-lg)[:5].tolist()
        v5 = lg[t5].tolist()
        gap = v5[0] - v5[1] if len(v5) > 1 else 0.0
        ok = t5[0] == REF[p]
        passed += ok
        row[p] = {"next": t5[0], "ref": REF[p], "ok": bool(ok), "gap": float(gap), "top5": t5}
        print(f"[A-int4] '{p}' next={t5[0]} ref={REF[p]} {'OK' if ok else 'MISMATCH'} gap={gap:.3f}", flush=True)
    print(f"[A-int4] ============ VERDICT {passed}/3 ============", flush=True)
    with open(os.path.join(HERE, "group_int4.json"), "w") as f:
        json.dump({"verdict": f"{passed}/3", "prompts": row}, f, indent=1)


if __name__ == "__main__":
    main()
