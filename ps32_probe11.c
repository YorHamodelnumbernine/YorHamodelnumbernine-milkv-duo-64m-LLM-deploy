/* ps32_probe11.c — two things:
   A) how many lmem bytes does alloc_ps32_matrix really reserve? (allocate a
      marker after it, compare addresses)
   B) does matrix l2g copy with FMT_I32 tgmem correctly de-interleave the
      ps32 buffer to a row-major int32 array for [1,896]?
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
  ...
  /* marker: small matrix allocated right after res */
  bmk1822_matrix_lmem_t *ml_mark = bmk1822_lmem_alloc_matrix(bmk,
    bmk1822_matrix_lmem_default_shape(bmk, 1, 16, FMT_I8), FMT_I8, 1);
  printf("ps32 res addr=%u size=%u | marker after res addr=%u -> reserved bytes=%d\n",
         ml_res->start_address, psz, ml_mark->start_address,
         ml_mark->start_address - ml_res->start_address);
  ...
  /* zero a generous region covering res */
  memset(va + 4096, 0, 16384); CVI_RT_MemFlush(rt, mem);
  bmk1822_tdma_g2l_general_copy(bmk, &(bmk1822_tdma_tg2l_general_copy_param_t){
    .src_base_reg_index = 0, .src_address = 4096, .dst_address = ml_res->start_address, .bytes = 16384 });
  ...
  /* readback B: matrix l2g copy with FMT_I32 tgmem */
  bmk1822_matrix_tgmem_t mg_o = {0, OUT_OFF, FMT_I32, {M,N}, {(uint32_t)N}};
  bmk1822_tdma_l2g_matrix_copy(bmk, &(bmk1822_tdma_l2tg_matrix_copy_param_t){ml_res, &mg_o});

  /* readback A: raw general copy for comparison (full likely region) */
  bmk1822_tdma_l2g_general_copy(bmk, &(bmk1822_tdma_l2tg_general_copy_param_t){
    ...
    .dst_address = OUT_OFF + 4096, .bytes = 4096 });
  ...
  int32_t *o = (int32_t*)(va + OUT_OFF);
  int bad = 0;
  for (int n = 0; n < N; n++)
    if (o[n] != host_ref[n]) { if (bad<5) printf("  col %d: got %d exp %d\n", n, o[n], host_ref[n]); bad++; }
  printf("FMT_I32 matrix l2g readback: %s (%d/%d bad)\n", bad==0?"MATCH":"MISMATCH", bad, N);

  /* also check where col 0..111 raw and find cols 112,896 in the 4096 raw dump */
  uint8_t *r = (uint8_t*)(va + OUT_OFF + 4096);
  printf("  raw res region[0..111]: "); for (int i=0;i<112;i++) printf("%02x ", r[i]);
  printf("\n");
