/* ps32_probe2.c — bmk1822 matrix_multiplication ps32 export capability probe (round 2).
   Focus: single-matmul K-slice. Is the int32 partial sum written anywhere in the
   ps32 matrix lmem buffer, and is it readable via raw general_copy?

   Also: verify that add_result=1 is rejected (assertion) even for ps32_mode=0.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void dump_raw(const char *tag, const uint8_t *b, int n) {
  printf("  [%s] ", tag);
  for (int i = 0; i < n; i++) { printf("%02x ", b[i]); if (i%8==7) printf("| "); }
  printf("\n");
  for (int off = 0; off + 4 <= n; off += 4) {
    int32_t v; memcpy(&v, b+off, 4);
    if (v != 0) printf("    off%02d: i32=%d\n", off, v);
  }
}

static void run_case(const char *name, int ps32_mode, int res_is_int8,
                     int use_ps32_matrix, fmt_t alloc_fmt, int k,
                     int L, int R, int dump) {
  CVI_RT_HANDLE rt;
  ...
  bmk1822_matrix_lmem_t *ml_res = use_ps32_matrix
    ? bmk1822_lmem_alloc_ps32_matrix(bmk, so, alloc_fmt, 1)
    : bmk1822_lmem_alloc_matrix(bmk, so, alloc_fmt, 1);

  printf("== %s ==\n  res: fmt=%s ps32mat=%d addr=%u sh{n=%u,c=%u,w=%u,col=%u} str{n=%u,c=%u,h=%u}\n",
         name, alloc_fmt==FMT_I8?"I8":alloc_fmt==FMT_BF16?"BF16":"other",
         use_ps32_matrix, ml_res->start_address,
         ml_res->shape.n, ml_res->shape.c, ml_res->shape.w, ml_res->shape.col,
  ...
  dump_raw("res", (uint8_t*)(va + OUT_OFF), nb);
...
int main(void) {
  printf("== ps32 export probe2 round2 ==\n");
  /* baseline normal int8 (expect first byte 127 sat) */
  run_case("nrm_i8", 0, 1, 0, FMT_I8, 32, 100, 100, 1);
  /* ps32_mode=1, ps32 matrix, alloc FMT_I8 */
  run_case("ps32m1_i8", 1, 1, 1, FMT_I8, 32, 100, 100, 1);
  /* ps32_mode=1, ps32 matrix, alloc FMT_BF16 */
  run_case("ps32m1_bf16", 1, 0, 1, FMT_BF16, 32, 100, 100, 1);
  /* ps32_mode=2, ps32 matrix, alloc FMT_BF16 */
  run_case("ps32m2_bf16", 2, 0, 1, FMT_BF16, 32, 100, 100, 1);
  /* ps32_mode=1, ps32 matrix, alloc FMT_I8, small values (no saturation) */
  run_case("ps32m1_i8_small", 1, 1, 1, FMT_I8, 32, 3, 3, 1);
  /* ps32_mode=1, ps32 matrix, alloc FMT_BF16, small values */
  run_case("ps32m1_bf16_small", 1, 0, 1, FMT_BF16, 32, 3, 3, 1);
  return 0;
}
