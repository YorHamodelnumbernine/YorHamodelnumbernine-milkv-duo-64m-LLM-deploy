/* ps32_probe10.c — locate specific output columns across FULL 32KB lmem.
   Columns: n=0 -> 1, n=111 -> 127, n=112 -> -1, n=895 -> -128.
   byte0 patterns: 0x01, 0x7f, 0xff, 0x80. Scan 32KB for each.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define OUT_OFF 32768   /* copy 32KB lmem here; neuron has room */
...
int main(void) {
  ...
  memset(right, 0, K*N);
  right[0*N + 0]   = 1;      /* col 0   -> 1   (0x01) */
  right[0*N + 111] = 127;    /* col 111 -> 127 (0x7f) */
  right[0*N + 112] = -1;     /* col 112 -> -1  (0xff) */
  right[0*N + 895] = -128;   /* col 895 -> -128(0x80) */
  ...
  /* zero 8KB around res */
  memset(va + 4096, 0, 8192); CVI_RT_MemFlush(rt, mem);
  bmk1822_tdma_g2l_general_copy(bmk, &(bmk1822_tdma_tg2l_general_copy_param_t){
    .src_base_reg_index = 0, .src_address = 4096, .dst_address = ml_res->start_address, .bytes = 8192 });
  ...
  printf("res addr=%u psz=%u sh{n=%u,c=%u,w=%u,col=%u} str{n=%u,c=%u,h=%u}\n",
         ml_res->start_address, psz, ml_res->shape.n, ml_res->shape.c, ml_res->shape.w,
         ml_res->shape.col, ml_res->stride.n, ml_res->stride.c, ml_res->stride.h);
  printf("lmem scan (full 32KB) for distinct byte0 values:\n");
  const int cols[] = {0, 111, 112, 895};
  const int vals[] = {0x01, 0x7f, 0xff, 0x80};
  for (int i = 0; i < 4; i++) {
    printf("  col %d (byte0=0x%02x) at lmem offsets:", cols[i], vals[i]);
    int cnt = 0;
    for (uint32_t off = 0; off < 32768; off++)
      if (r[off] == (uint8_t)vals[i]) { if (cnt < 12) printf(" %u", off); cnt++; }
    printf("  total=%d\n", cnt);
  }
  printf("  any nonzero in res region [%u..%u):\n", ml_res->start_address, ml_res->start_address + psz);
  for (uint32_t off = ml_res->start_address; off < ml_res->start_address + psz; off++)
    if (r[off]) printf("    lmem %u = 0x%02x\n", off, r[off]);
