...
### 7.5 方向 2（FFN-only INT4）快速验证：判负（2026-08-13）

FFN-only 快速验证完成（`REPORT_FFNONLY_INT4_20260813.md`，3 轮中位，三轮全复现、
确定性、非代码 bug），结论与 §7.2 判定一致：

| 配置 | next_token | prefill gap | Wt load | Total |
|---|---|---|---|---|
| FFN-only G64 | 18164 ✗ | 0.0 | 2,168 ms (−44%) | 32,304 ms (−25%) |
| FFN-only G32 | 15247 ✗ | 0.9 | 2,272 ms | 32,824 ms |
| FFN-only G16 | 6271 ✗ | 6.1 | 2,201 ms | 32,116 ms |

- 性能达标：Wt load 3.86s→2.17s（−44%，优于预期）、Total −25%。
- 质量全败：G64/G32 下 prefill top-2 并列（gap 0.0/0.9，logit 塌缩）；G16 下 gap 恢复
  健康（6.1）但 argmax 仍错（6271）。G 64→16 仅把 FFN rms 4.2→3.2，不足以保 top-1。
- **无推荐组大小**：最小 G16（字节节省缩水到 −28%）仍判负 → 无质量可行点。

**最终结论：INT4 整线（全量 + FFN-only）正式确认不可行，维持 INT8 生产路径。**
