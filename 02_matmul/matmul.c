/* TPU matrix_multiplication: INT8 matmul left[2x2] * right[2x2] = res[2x2] */
...
static const int8_t left[M*K]   = {1, 2, 3, 4};     /* [[1,2],[3,4]] */
static const int8_t right[K*N]  = {1, 2, 3, 4};     /* [[1,2],[3,4]] */
static const int8_t exp[M*N]    = {7, 10, 15, 22};  /* [[7,10],[15,22]] */
static const int8_t exp_bias[M*N]={8,11,16,23};      /* result + 1 bias */
...
int main() {
  ...
  /* bias at offset 32 — row of N values repeated for M rows */
  int8_t bias_data[M*N]; for(int i=0;i<M*N;i++) bias_data[i]=1;
  ...
  /* G2L */
  cvk_tdma_g2l_matrix_copy_param_t g2l_l = {&mg_l, ml_l};
  cvk_tdma_g2l_matrix_copy_param_t g2l_r = {&mg_r, ml_r};
  cvk_tdma_g2l_matrix_copy_param_t g2l_b = {&mg_b, ml_bias};
  cvk->ops->tdma_g2l_matrix_copy(cvk, &g2l_l);
  cvk->ops->tdma_g2l_matrix_copy(cvk, &g2l_r);
  cvk->ops->tdma_g2l_matrix_copy(cvk, &g2l_b);

  /* TIU matmul: res = left * right + bias */
  cvk_tiu_matrix_multiplication_param_t p = {
    ...
    .add_result = 1,   /* add bias to result */
    .ps32_mode = 0,
  };
  cvk->ops->tiu_matrix_multiplication(cvk, &p);

  /* L2G */
  cvk_tdma_l2g_matrix_copy_param_t l2g = {ml_res, &mg_o};
  cvk->ops->tdma_l2g_matrix_copy(cvk, &l2g);
  ...
  fprintf(stderr,"[dbg] matmul:"); for(int i=0;i<M*N;i++) fprintf(stderr,"%d,",v[48+i]);
  fprintf(stderr,"\n");
  int errs = tpu_check_i8("matmul", v+48, exp_bias, M*N, M*N);
  printf("  time: %.2f us\n", t_ns/1000.0);
