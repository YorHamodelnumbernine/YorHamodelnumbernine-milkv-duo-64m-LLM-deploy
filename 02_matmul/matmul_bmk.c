/* TPU matrix multiplication benchmark — bmk1822 API.
   INT8 matmul left[2x2] * right[2x2] = res[2x2].
   left=[1,2;3,4], right=[1,2;3,4] => res=[7,10;15,22] */
...
int main() {
  ...
  static const int8_t left[4]  = {1, 2, 3, 4};      /* [[1,2],[3,4]] row-major */
  static const int8_t right[4] = {1, 2, 3, 4};      /* [[1,2],[3,4]] row-major */
  static const int8_t expected[4] = {7, 10, 15, 22};
  ...
  errs = 0;
  for (int i = 0; i < 4; i++) { if (v[32+i] != expected[i]) errs++; }
  printf("  [matmul_bmk] got=[%d,%d,%d,%d] exp=[%d,%d,%d,%d]  %s  time: %.2f us\n",
    v[32],v[33],v[34],v[35], expected[0],expected[1],expected[2],expected[3],
