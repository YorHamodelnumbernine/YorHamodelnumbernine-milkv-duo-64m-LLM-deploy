/* ps32_probe19.c — capacity/timing proof for forced c=1 ps32 with full model
   widths N=256/448/896, K=32 group.  Verifies single-call full-N ps32 output
   fits 32KB LMEM (right K*N + res 4*N + left K) and times one batched submit
   with N matmuls packed into one cmdbuf (realistic engine usage).
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int run_forced(int K, int N, int reps) {
  ...
  uint32_t lmem_need = K + K*N + 4*N;
  printf("[F%d] lmem need ~%u (32KB=%u) %s\n", N, lmem_need, 32768,
         lmem_need <= 32768 ? "FIT" : "OVER");
  ...
  if (!ml_l || !ml_r || !ml_res) {
    printf("[F%d] alloc fail\n", N);
    return -1;
  }
  uint32_t psz = bmk1822_lmem_ps32_matrix_to_size(bmk, so, FMT_I8, 1);
  uint32_t rsz = bmk1822_lmem_matrix_to_size(bmk, sr, FMT_I8, 1);
  printf("[F%d] res@%u psz=%u right@%u rsz=%u left@%u\n", N,
         ml_res->start_address, psz, ml_r->start_address, rsz, ml_l->start_address);
  ...
  /* pack reps identical matmuls into one cmdbuf (readback after all) */
  ...
  printf("[F%d] ok=%d/%d  reps=%d total=%.3fms per-matmul=%.3fms\n",
         N, ok, N, reps, ms, ms/reps);
  free(host);
...
int main(void) {
  printf("== probe19: forced c=1 ps32 full-N capacity (K=32) ==\n");
  run_forced(32, 256, 100);
  run_forced(32, 448, 100);
  run_forced(32, 896, 100);
  return 0;
}
